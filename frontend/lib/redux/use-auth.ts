...
export const useAuthSlice = () => {
  ...
  const updateCurrentUser = async (userData: User) => {
    ...
    return {
      ...
      data: result.payload,
      error: updateCurrentUserThunk.rejected.match(result)
        ? result.payload
        : null,
    };
  };
  ...
  const verifyEmail = async (token: string) => {
    ...
    return {
      success: verifyEmailThunk.fulfilled.match(result),
      data: result.payload,
      error: verifyEmailThunk.rejected.match(result)
  ...
  const resendVerification = async (email: string) => {
    ...
    return {
      success: resendVerificationThunk.fulfilled.match(result),
      data: result.payload,
      error: resendVerificationThunk.rejected.match(result)
        ? (result.payload as AuthError)
        : null,
    };
  };
